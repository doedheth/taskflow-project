...
echo "🛡️ Keeping services online during build (min-downtime)..."
...
echo "🔄 Restarting Services (graceful)..."
...
# Server: prefer zero-downtime reload (cluster if first run)
...
if pm2 describe "$CLIENT_APP" >/dev/null 2>&1; then
